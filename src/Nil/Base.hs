...
modpow b e m | m <= 0    = die $ "non-positive modulo used: " ++ show m
...
primep'fermat b n | n < 0     = die "Negative argument"
                  | n == 1    = False
                  | n == 2    = True
...
sqrt'zpz n p
 ...
 where
  loop _M c t _R
    ...
    = Just _R
    | otherwise
    = let i = die ("cannot find proper i from t: " ++ show t)
