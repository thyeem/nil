...
import           Nil.Circuit
import           Nil.Reorg      ( amp'wirep, frozen'wirep, reorg'circuit,
                                  shift'wirep )
...
export'graph file script = do
  (e, _, err) <- readProcessWithExitCode "dot" ["-Tpdf", "-o" ++ file] script
  when (e /= ExitSuccess) (die $ "Failed to convert - " ++ err)

-- | Convert a circuit into a dot-language script
write'dot :: (Eq f) => String -> Circuit f -> String
write'dot header circuit =
  unwords ["digraph G {", header, write'gates circuit, "}"]

dot'header :: String
dot'header = "size=\"32,32\"; rankdir=LR;"

write'gates :: (Eq f) => Circuit f -> String
write'gates Circuit {..} = unwords $ write'gate c'pubs c'privs <$> c'gates

write'gate :: (Eq f) => [String] -> [String] -> Gate f -> String
write'gate instances witnesses Gate {..}
  | key g'lwire == "return" = unwords [edge g'rwire g'lwire, style g'lwire]
  | otherwise =
    unwords
  ...
  where
    ...
    edge from to =
      unwords [key from, "->", key to, "[color=" ++ edge'color from ++ "];"]
    edge'color wire
      | w'recip wire = "red"
      | out'wirep wire = "blue"
      | otherwise = "black"
    ...
    style wire
      ...
      | const'wirep wire =
        unwords [key wire, "[shape=egg,style=dashed,label=Const,fontsize=18];"]
      ...
      | w'key wire `elem` witnesses =
        unwords
          [key wire, "[shape=egg,style=filled,fillcolor=pink,fontsize=30];"]
      | otherwise = mempty
