...
import           Nil.Circuit
import           Nil.Poly         ( lagrangepoly, (<.>), (<|.>), (|/) )
import           Nil.Utils        ( Pretty (..), pfold, (<%>) )
...
r1cs'from'circuit circuit@Circuit {..} =
  r1cs'A `par` r1cs'B `par` r1cs'C `pseq`
  R1CS {r1cs'A, r1cs'B, r1cs'C, r1cs'num'inst = length c'pubs}
  where
...
l'from'gate keys g@Gate { g'op = op
...
r'from'gate keys g@Gate { g'op = op
                        , g'rwire = Wire rkey _ _ rval
...
o'from'gate keys Gate {g'op = op, g'owire = Wire okey _ _ oval} =
  [coeff key | key <- keys]
  where
    coeff key
      | op == End && key == okey = 2
...
qap'from'r1cs R1CS {..} =
  qap'V `par` qap'W `par` qap'Y `par` qap't `pseq`
  QAP {qap'V, qap'W, qap'Y, qap't, qap'num'inst = r1cs'num'inst}
  where
...
qap'from'circuit :: (Eq f, Fractional f, Enum f, NFData f) => Circuit f -> QAP f
