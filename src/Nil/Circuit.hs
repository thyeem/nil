...
import           Nil.Utils       ( Pretty (..), bytes'from'str, die,
                                   hex'from'bytes, int'from'bytes, sha256 )
...
init'state pubs privs =
  ( rep (foldl' go [] wires)
  , fromList $ (\wire -> (w'key wire, wire)) <$> (unit'var <$> pubs ++ privs))
  where
    rep =
      \case
    ...
    wires =
      concat
        [[unit'var p, set'val (-1) . unit'var $ p] | p <- pubs, p /= return'key]
  ...
  where
    go seq' =
      \case
        ...
        o@Out {} -> o : seq'
        Null -> seq'
        Seq (Out Return _) Null ->
          die "Error, must be only one return statement."
        e -> die $ "Error, found invalid statement:\n" ++ pretty e
  ...
  \case
    ...
    Out Return x ->
      let (gates, wmap) = expr' state x
          eval'out
            | null gates =
              die "Error, not found gate. There must be at least one."
            | otherwise = g'owire . head $ gates
       in ( Gate End (unit'var return'key) eval'out (unit'var end'key) : gates
          , wmap)
...
expr'if state a b c =
  let out = g'owire . head . fst
      cond'a = expr' state a
      a'mul'b =
        gate' Star (expr'wire state a) (expr'wire cond'a b) (expr' cond'a b)
      neg'a = gate' Star (val'const (-1)) (out cond'a) a'mul'b
      neg'a'one = gate' Plus unit'const (out neg'a) neg'a
      neg'a'one'mul'c =
        gate' Star (out neg'a'one) (expr'wire neg'a'one c) (expr' neg'a'one c)
...
gate' op lwire rwire (gates, wmap) =
  case op of
...
wire'keys Circuit {..} =
  const'key : concat [sort c'pubs, sort c'privs, w'key . g'owire <$> c'gates]
