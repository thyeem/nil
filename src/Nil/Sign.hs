...
import           Nil.Circuit
import           Nil.Curve        ( Curve, Point (..), c'g, (~*) )
import           Nil.Data         ( NIL (NIL), lift, nil, unil, unil' )
import           Nil.Ecdata       ( BN254, BN254'G2, Fr, G1, bn254'g1, bn254'g2,
                                    bn254'gt )
import           Nil.Eval         ( eval'circuit, extend'gate, extend'wire,
                                    statement, wmap'fromList, (~~), (~~~) )
import           Nil.Field        ( Extensionfield (..), Field, unef )
import           Nil.Pairing      ( pairing )
import           Nil.Reorg
import           Nil.Utils        ( Pretty (..), bytes'from'hex, bytes'from'int,
                                    bytes'from'str, deep, die, foldM',
                                    hex'from'bytes, info'io, ranf, sha256,
                                    stderr, (<%>), (|=|) )
...
instance (Show q, Pretty q, Field q) => Pretty (Nilkey i j q) where
...
{-# INLINE nil'init #-}
update'nilkey :: (Integral r, Field q) => r -> r -> Nilkey i j q -> Nilkey i j q
...
nil'sign ::
  ...
  -> Nilsig i j r q
  -> IO (Nilsig i j r q)
nil'sign _ secrets sig@Nilsig {..} = do
  alpha <- ranf
  gamma <- ranf
  let omap = omap'from'gates . c'gates $ n'circuit
      ...
      nilkey =
        update'nilkey (recip alpha) alpha . update'nilkey gamma (recip gamma) $
  ...
  pure $
    sig {n'key = nilkey, n'circuit = n'circuit {c'gates = sort'omap collapsed}}
...
backprop alpha gamma gmap omap g
  ...
  | otherwise = do
    beta <- ranf
    let acc =
          if prin'amp'p omap g
            then updater gamma g omap
            else omap
    pure $
      foldl'
...
sign'gate secrets gmap omap g = do
  delta <- ranf
  epsilon <- ranf
  let (entry, _) = either'by entry'wirep g
      NIL c _ = w'val entry
  if member (w'key entry) secrets
    then do
      let lifter =
          ...
          shifted
            | const'wirep entry =
              nil c delta * (nil c 1 + nil c epsilon / secret)
            | otherwise = nil c delta * (secret + nil c epsilon)
       in pure . nilify False False multiplier (get'amp gmap g) .
...
nilify leftp closep val g omap =
  let gate@Gate {g'lwire = gl, g'rwire = gr} = omap >>> g
      finish =
        if closep
          then freeze . set'recip False
          else set'recip False
      gate' =
        if leftp
    ...
    update (wmap, omap) gate@Gate {..} =
      let g = gate {g'lwire = rep g'lwire, g'rwire = rep g'rwire}
          rep wire
            ...
            | otherwise = wire
       in if reducible g
            then ( wmap <~ (w'key g'owire, freeze . val'const $ eval' g)
...
hash'amps omap =
  ...
  where
    ...
    salt =
      case find (prin'amp'p omap) amps of
...
hash'nilkey curve (phi, chi) =
  foldl' ((sha256 .) . append) mempty (bytes'from'int . toInteger <$> lambda)
  where
...
nil'test verbose language wmap
  -- Language
 = do
  when verbose $ do
    stderr mempty
...
which'signed Nilsig {..} mode = foldl' rep which entries
  where
    items
    ...
    entries =
      w'key . g'lwire <$> find'entries (omap'from'gates . c'gates $ n'circuit)
    which =
      fromList $
      (\x ->
         if x == return'key
