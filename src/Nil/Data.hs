...
instance (Integral r, Field r, Integral q, Field q) => Num (NIL i r q) where
  (+) = add
  {-# INLINE (+) #-}
  (-) = sub
  {-# INLINE (-) #-}
...
instance (Integral r, Field r, Integral q, Field q) =>
         Fractional (NIL i r q) where
  recip = recip'
  {-# INLINE recip #-}
  (/) !a !b = a * recip b
  {-# INLINE (/) #-}
...
instance (Eq r, Ord r, Integral r, Integral q, Field q) => Ord (NIL i r q) where
  a <= b = unil a <= unil b
...
nil' :: (Integral q, Field q, Field q) => Curve i q -> Point i q -> NIL i r q
...
-- | mul
mul ::
     (Integral r, Field q, Integral q, Show r)
  => NIL i r q
  -> NIL i r q
  -> NIL i r q
mul a@(NIL c a_) b@(NIL _ b_) =
  case (a_, b_) of
    (U x, U y) -> nil c (x * y)
