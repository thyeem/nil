...
import           Nil.Utils       ( bytes'from'int, die, int'from'bytes, sha256 )
...
wire'vals c wmap circuit@Circuit {..} =
  unil . w'val . (eval'circuit xmap xcirc ~>) <$> wire'keys circuit
  where
...
vec'fromWmap wmap =
  w'val . (wmap ~>) <$> filter (/= const'key) (sort . keys $ wmap)
...
wmap'fromList =
  foldl'
    (\wmap (key, val) -> wmap <~ (key, set'val val . unit'var $ key))
    (mempty <~~ unit'const)
  ...
  where
    ...
    val wire =
      case wmap ~~ wire of
        ...
        _ ->
          die $
          unwords ["Error, used", label, "on non-scalar wire:", w'key wire]
...
eval'hash hasher wmap Gate {..} = wmap <~~ (hash (wmap ~~ g'rwire) ~~~ g'owire)
  where
    ...
    hash =
      nil c .
      fromIntegral .
      int'from'bytes . hasher . bytes'from'int . fromIntegral . unil
